define([
    ...
    var AbstractSurfaceShapeEditTool = function(wwd, shape) {
        ...
        var headingHandlePosition = this._computeHeadingHandlePosition();
        ...
        this._toHeadingLine = new Path([
            heightHandlePositions[1],
            headingHandlePosition
        ]);
        this._headingHandle = new PointPlacemark(headingHandlePosition);

        this._headingHandle.interiorColor = AbstractSurfaceShapeEditTool.HEADING_HANDLE_COLOR;
        this._headingHandle.highlightAttributes = new PointPlacemarkAttributes(this._headingHandle.attributes);
        this._headingHandle.highlightAttributes.interiorColor = AbstractSurfaceShapeEditTool.HIGHLIGHTED_HANDLE_COLOR;

        this._toHeadingLine.attributes.outlineColor = AbstractSurfaceShapeEditTool.HEADING_HANDLE_COLOR;
        this._toHeadingLine.attributes.outlineWidth = 2;
        ...
        this.addEditRenderable(this._toHeadingLine);
    ...
    AbstractSurfaceShapeEditTool.HEADING_HANDLE_COLOR = Color.LIGHT_GRAY;
    ...
    AbstractSurfaceShapeEditTool.HEADING_HANDLE_DISTANCE_PX = 25;
    ...
    AbstractSurfaceShapeEditTool.prototype._allHandles = function() {
        return [this._headingHandle, this._centerHandle].concat(this._heightHandles).concat(this._widthHandles);
    };
    ...
    AbstractSurfaceShapeEditTool.prototype._computeHeadingHandlePosition = function() {
        var topLoc = this._fromCenterWithHeading(0, this.getHalfHeight());
        var eyePt = this.wwd.drawContext.eyePoint;
        var topPt = this.wwd.drawContext.surfacePointForMode(topLoc.latitude, topLoc.longitude, 0, WorldWind.CLAMP_TO_GROUND, new Vec3(0, 0, 0));
        var dist = AbstractSurfaceShapeEditTool.HEADING_HANDLE_DISTANCE_PX*this.wwd.drawContext.pixelSizeAtDistance(eyePt.distanceTo(topPt));
        return this._fromCenterWithHeading(0, this.getHalfHeight() + dist);
    };

    AbstractSurfaceShapeEditTool.prototype._beforeDrawFrame = function() {
        var heightHandlePositions = this._computeHeightHandlePositions();
        this._headingHandle.position = this._computeHeadingHandlePosition();
        this._toHeadingLine.positions = [
            heightHandlePositions[1],
            this._headingHandle.position
        ];
    };
